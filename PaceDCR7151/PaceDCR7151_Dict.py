                ...
                if (upgrade == 1):
                    while(Tentativas_Act < Num_Tentativas_Atualizcao):
                        if (Tentativas_Act == 0):
                            ...
                            TEST_CREATION_API.write_log_to_file("Passou por Aqui")
                            time.sleep(1)
                        ...
                        if (NOS_API.is_signal_present_on_video_source()):                        
                            ...
                            if(compare_result == 1 or compare_result == 2 or compare_result == 3 or compare_result == 4 or compare_result >= 47):
                                ...
                                if (upgrade == 0):
                                    ...
                                    upgrade = 2
                                    test_result = "FAIL"
                            elif(compare_result == 5 or compare_result == 6 or compare_result >= 25 and compare_result < 47):
                ...
                while (upgrade < 2):
                            ...
                            while (delta_time_standby < SW_UPGRADE_TIMEOUT):
                                ...
                                if (NOS_API.is_signal_present_on_video_source()):                        
                                    ...
                                    compare_result = NOS_API.wait_for_multiple_pictures(
                                                                        ...
                                                                        [80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 30, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80])                       
            
                                    if(compare_result == 0 or compare_result == 1 or compare_result == 2 or compare_result >= 27):
                                        NOS_API.test_cases_results_info.channel_boot_up_state = False 
                                        time.sleep(5)
                                        test_result_boot = True
                                        upgrade = 2
                                        ...
                                        return                           
                                    elif(compare_result == 3 or compare_result == 4 or compare_result >= 6 and compare_result < 27):
                                        NOS_API.test_cases_results_info.channel_boot_up_state = True
                    ...
                    if (NOS_API.is_signal_present_on_video_source() and test_result_boot):
                        ...
                        if (NOS_API.test_cases_results_info.channel_boot_up_state):
                            ...
                            result = NOS_API.wait_for_multiple_pictures(["sw_version_ref", "sw_version_ref_old", "sw_version_black_ref", "Sw_Version_Black_Dict_ref", "Sw_Version_Dict_ref", "Sw_Version_Dict_2_ref"], 5, ["[SW_VERSION_SCREEN]", "[SW_VERSION_SCREEN_Two]", "[SW_VERSION_SCREEN]", "[Sw_Version_Menu_Dict]", "[Sw_Version_Menu_Dict]", "[Sw_Version_Menu_Dict]"], [70, 70, 70, 70, 70, 70])
                            if(result == -2):
                                TEST_CREATION_API.write_log_to_file("STB lost Signal.Possible Reboot.")
                                NOS_API.update_test_slot_comment("Error code = " + NOS_API.test_cases_results_info.reboot_error_code \
                                                        + "; Error message: " + NOS_API.test_cases_results_info.reboot_error_message)
                                ...
                                if (NOS_API.check_ber(ber_value, BER_VALUE_THRESHOLD)):
                                    ...
                                    if not(NOS_API.test_cases_results_info.channel_boot_up_state):      
                                        ...
                                        if (ch_installation_fti == True):
                                            ...
                                            result = NOS_API.wait_for_multiple_pictures(["sw_version_ref", "sw_version_ref_old", "Sw_Version_Dict_ref", "Sw_Version_Dict_2_ref"], 5, ["[SW_VERSION_SCREEN]", "[SW_VERSION_SCREEN_Two]", "[Sw_Version_Menu_Dict]", "[Sw_Version_Menu_Dict]"], [70, 70, 70, 70])
                                            if(result == -2):
                                                TEST_CREATION_API.write_log_to_file("STB lost Signal.Possible Reboot.")
                                                NOS_API.update_test_slot_comment("Error code = " + NOS_API.test_cases_results_info.reboot_error_code \
                                                                        + "; Error message: " + NOS_API.test_cases_results_info.reboot_error_message)
        ...
        ##############################################################################################################################################################################################################################
            ...
            if(Input_Signal_TestCase): 
                ...
                if (NOS_API.is_signal_present_on_video_source()):
                    ...
                    #MAC_SCREEN
                    result = NOS_API.wait_for_multiple_pictures(["mac_screen_ref", "mac_screen_ref1", "Mac_Screen_Dict_ref"], 10, ["[MAC_SCREEN]", "[MAC_SCREEN]", "[MAC_SCREEN_DICT]"], [70, 70, 70])
                    if(result == -2):
                        TEST_CREATION_API.write_log_to_file("STB lost Signal.Possible Reboot.")
                        NOS_API.update_test_slot_comment("Error code = " + NOS_API.test_cases_results_info.reboot_error_code \
                                                + "; Error message: " + NOS_API.test_cases_results_info.reboot_error_message)
                                    ...
                                    if (rx_value > RX_THRESHOLD_LOW and rx_value < RX_THRESHOLD_HIGH):
                                        if(downloadstream_snr_value >= DOWNLOADSTREAM_SNR_THRESHOLD):
                                            if (ip_adress  != "0.0.0.0"):
                                                ...
                                                while not (TEST_CREATION_API.compare_pictures("sc_info_ref1", "Right_Place", "[CAS_ID_RP]") or TEST_CREATION_API.compare_pictures("sc_info_ref2", "Right_Place", "[CAS_ID_RP]") or TEST_CREATION_API.compare_pictures("sc_info_ref3", "Right_Place", "[CAS_ID_RP]") or TEST_CREATION_API.compare_pictures("sc_info_ref4", "Right_Place", "[CAS_ID_RP]") or TEST_CREATION_API.compare_pictures("Sc_Info_Dict_ref", "Right_Place", "[CAS_ID_RP_DICT]")):
                                                    ##Perform Down
                    ...
                    ################################################################################ Buttons/Led's Test ###################################################################################################### 
                                ...
                                if(test_result_telnet):
                                    ...
                                    if(test_result_ButtonLeds):
                                        ...
                                        video_result_2 = NOS_API.compare_pictures("Factory_Reset_Dict_ref", "Factory_Reset", "[Factory_Check]")
                                        video_result_3 = NOS_API.compare_pictures("Factory_Reset_Black_Dict_ref", "Factory_Reset", "[Factory_Check]")
                                        
                                        if not(video_result >= TEST_CREATION_API.DEFAULT_HDMI_VIDEO_THRESHOLD or video_result_1 >= TEST_CREATION_API.DEFAULT_HDMI_VIDEO_THRESHOLD or video_result_2 >= TEST_CREATION_API.DEFAULT_HDMI_VIDEO_THRESHOLD or video_result_3 >= TEST_CREATION_API.DEFAULT_HDMI_VIDEO_THRESHOLD):
                                            TEST_CREATION_API.send_ir_rc_command("[EXIT_ZON_BOX]")
                                            TEST_CREATION_API.send_ir_rc_command("[Factory_Reset]")
                                            ...
                                            video_result_4 = NOS_API.compare_pictures("Factory_Reset_Dict_ref", "Factory_Reset_1", "[Factory_Check]")
                                            video_result_5 = NOS_API.compare_pictures("Factory_Reset_Black_Dict_ref", "Factory_Reset_1", "[Factory_Check]")
                                            
                                            if not(video_result_2 >= TEST_CREATION_API.DEFAULT_HDMI_VIDEO_THRESHOLD or video_result_3 >= TEST_CREATION_API.DEFAULT_HDMI_VIDEO_THRESHOLD or video_result_4 >= TEST_CREATION_API.DEFAULT_HDMI_VIDEO_THRESHOLD or video_result_5 >= TEST_CREATION_API.DEFAULT_HDMI_VIDEO_THRESHOLD):
                                                TEST_CREATION_API.write_log_to_file("Navigation to resumo screen failed")
                                                NOS_API.set_error_message("Navegação")
                                                NOS_API.update_test_slot_comment("Error code = " + NOS_API.test_cases_results_info.navigation_error_code \
                                                                                    + "; Error message: " + NOS_API.test_cases_results_info.navigation_error_message) 
